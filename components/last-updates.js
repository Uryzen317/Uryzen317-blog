  ...
  return (
    <div className="main-container">
      ...
      <div className="header bg-gray-950 h-6 w-full px-1 flex items-center">
        <p className="text-sm font-bold text-gray-300">Last updates</p>
      </div>
        ...
        <Link href={"topic/" + topic.title}>
          <div className="item bg-gray-800 h-6 cursor-pointer w-full px-1 flex items-center justify-between border-b border-gray-600 hover:bg-gray-700">
            ...
            <p className="text-xs text-gray-300 font-bold">
              @ {new Date(topic.updatedAt).getFullYear()}/
              {new Date(topic.updatedAt).getMonth()}/
              {new Date(topic.updatedAt).getDate()}-
              {new Date(topic.updatedAt).getHours()}:
              {new Date(topic.updatedAt).getMinutes()}:
              {new Date(topic.updatedAt).getSeconds()}
            </p>
          </div>
        </Link>
      ))}
